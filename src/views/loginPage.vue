<script setup>
  ...
  function re_judge(){
    if(re_password.value == '' ||  re_username.value == ''){
      ...
      axios.post("http://10.100.236.20:8080/register",re_userinfo,{
  ...
  function log_judge(){
    if(log_password.value == '' ||  log_username.value == ''){
      ...
      axios.post("http://10.100.236.20:8080/login",log_userinfo,{
        headers: {'Content-Type': 'application/json;charset=UTF-8', 'Accept': '*'},
        timeout: 30000
      }).then(res => {
        console.log(res)
    ...
    <div class="container_overlay">
      <div class="overlay">
        <div class="overlay_panel overlay--left">
          <button class="btn" @click="()=>{right_panel_active = false}">已有账号，直接登录</button>
        </div>

        <div class="overlay_panel overlay--right">
          <button class="btn" @click="()=>{right_panel_active = true}">没有账号，点击注册</button>
        </div>
      </div>
    </div>
